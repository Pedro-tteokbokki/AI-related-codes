...
from Pedro_package.core import Pedro_Function, Pedro_Variable, as_variable, as_array
from dezero import cuda, utils


class Sin(Pedro_Function):
    def forward(self, x):
        y = np.sin(x)
        return y
...
class Transpose(Pedro_Function):
...
class GetItemGrad(Pedro_Function):
...
class BroadcastTo(Pedro_Function):
    def __init__(self, shape):
        self.shape = shape
...
class LogSoftmax(Pedro_Function):
    def __init__(self, axis=1):
        self.axis = axis
...
class LeakyReLU(Pedro_Function):
...
class MeanSquaredError(Pedro_Function):
...
class SoftmaxCrossEntropy(Pedro_Function):
...
def accuracy(y, t):
    ...
    result = (pred == t.data)
    acc = result.mean()
    return Pedro_Variable(as_array(acc))
...
class BatchNorm(Pedro_Function):
