...
import numpy as np
from Pedro_package import as_variable
from Pedro_package import Pedro_Variable
from Pedro_package import cuda
...
def plot_dot_graph(output, verbose=True, to_file='graph.png'):
    ...
    tmp_dir = os.path.join(os.path.expanduser('~'), '.Pedro_package')
...
def numerical_grad(f, x, *args, **kwargs):
    ...
    x = x.data if isinstance(x, Pedro_Variable) else x
    xp = cuda.get_array_module(x)
    if xp is not np:
    ...
    while not it.finished:
        ...
        if isinstance(y1, Pedro_Variable):
            y1 = y1.data
        y1 = y1.copy()

        x[idx] = tmp_val - eps
        y2 = f(x, *args, **kwargs)  # f(x-h)
        if isinstance(y2, Pedro_Variable):
            y2 = y2.data
        y2 = y2.copy()
...
def array_equal(a, b):
    ...
    """
    a = a.data if isinstance(a, Pedro_Variable) else a
    b = b.data if isinstance(b, Pedro_Variable) else b
